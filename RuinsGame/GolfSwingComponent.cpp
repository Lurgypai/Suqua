...
GolfSwingComponent::GolfSwingComponent(EntityId id_) :
	id{ id_ },
	hitboxId{ 0 },
	delay{ 0 },
	duration{ 0 },
...
void GolfSwingComponent::update()
{
	...
	if (!isActive_) {
		if (cont.toggled(ControllerBits::BUTTON_12) && cont[ControllerBits::BUTTON_12]) {
			isActive_ = true;
			tick = 0;
			gfxComponent->playAnimation("golf_swing", false);
